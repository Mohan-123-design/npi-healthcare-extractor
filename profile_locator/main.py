...
from modules.perplexity_search import perplexity_search
from modules.web_utils import logger

load_dotenv()

MAX_WORKERS = int(os.getenv("MAX_WORKERS", "8"))
BATCH_SIZE = int(os.getenv("BATCH_SIZE", "4"))          # small threaded batch (unchanged)
BIG_BATCH_SIZE = int(os.getenv("BIG_BATCH_SIZE", "1000"))  # new: big batch size for user confirmation
...
# Stop run when Perplexity has fatal errors, unless disabled via .env
STOP_ON_PERPLEXITY_ERROR = (
    os.getenv("STOP_ON_PERPLEXITY_ERROR", "true").lower() == "true"
)
FATAL_PPLX_STATUSES = {
    "quota_exceeded_all_keys",
    "auth_error_all_keys",
    "auth_missing",
    "server_error",
    "network_error",
...
def _extract_workplace_name_from_text(text: str) -> str:
    """
    Given the plain-text response from Google/Perplexity, extract a single
    workplace/organization name string.
    """
    if not text:
        return ""
...
def process_single(row):
    ...
    # Perplexity with retries, but stop early if all keys exhausted
    p = {"found": False, "text": "", "error": None, "error_type": None}
    for attempt in range(RETRY_COUNT + 1):
        p = perplexity_search(npi, email, name, work_address)
        err_type = p.get("error_type")
        # If success or non-error or all keys exhausted, break
        if p.get("found") or not p.get("error") or (
            err_type
            and err_type in (
                "quota_exceeded_all_keys",
                "auth_error_all_keys",
    ...
    p_name = _extract_workplace_name_from_text(p_text)
    result["perplexity_name"] = p_name or "N/A"
    result["perplexity_status"] = p.get("error_type") or "ok"

    # Final workplace name: Perplexity > Google > N/A
    final_name = p_name or g_name or ""
    if not final_name:
        final_name = "N/A"
                    ...
                    for fut in concurrent.futures.as_completed(futures):
                        try:
                            ...
                            logger.info(
                                ...
                                f"(Perplexity status: {res.get('perplexity_status')})"
                            )

                            status = res.get("perplexity_status") or "ok"
                            if (
                                STOP_ON_PERPLEXITY_ERROR
                                and status in FATAL_PPLX_STATUSES
                                and fatal_reason is None
                            ):
                                fatal_reason = (
                                    f"Stopping run: Perplexity error '{status}' "
                                    f"for NPI {res.get('npi_id')}. "
                                    "Check API credits/keys and rerun."
                                )
